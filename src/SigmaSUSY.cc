...
void Sigma2qqbar2chi0chi0::initProc() {
  ...
  name = "q qbar' -> " + particleDataPtr->name(id3) + " "
...
void Sigma2qg2chi0squark::initProc() {
  ...
  // Construct name of process.
  if (id4 % 2 == 0) {
    name = "q g -> " + particleDataPtr->name(id3) + " "
...
void Sigma2qq2squarksquark::initProc() {
  ...
  // Derive name
  name = "q q' -> "+particleDataPtr->name(abs(id3Sav))+" "
...
double Sigma2qq2squarksquark::sigmaHat()
{
  ...
  {
      m2Neutralino[iNeut] = pow2(particleDataPtr->m0(coupSUSYPtr->idNeut(iNeut)));
  }
  
  m2CHargino[1] = pow2(particleDataPtr->m0(coupSUSYPtr->idChar(1)));
  m2CHargino[2] = pow2(particleDataPtr->m0(coupSUSYPtr->idChar(2)));
  ...
  if (isUD)
  {
    ...
    double G_LL = norm(LsuuG[ind4][ind1] * LsddG[ind3][ind2]);
    ...
    sigmaGu += sigmaGlu / pow2(uGlu) * (G_LL + G_LR + G_RL + G_RR);
  ...
  else
  {
    ...
    for (int k=1;k<=nNeutralinos && !onlyQCD;k++) 
    {
      ...
      {
            // kl-dependent factor for LL and RR contributions
            double facMS = sH * particleDataPtr->m0(coupSUSYPtr->idNeut(k)) * particleDataPtr->m0(coupSUSYPtr->idNeut(l));
      ...
      double facMS = sH * particleDataPtr->m0(coupSUSYPtr->idNeut(k)) * particleDataPtr->m0(1000021);
...
double Sigma2qq2squarksquark::sigmaPDF2()
{
  ...
  int nQuarkIn        = settingsPtr->mode("PDFinProcess:nQuarkIn");
  // int nQuarkIn = 6;

  // get K factor, multiplying resolved processes. (But not here for MPI.)
  double Kfactor = settingsPtr->parm("SigmaProcess:Kfactor");
  ...
  for (int id = -nQuarkIn; id <= nQuarkIn; ++id)
  {
    ...
    pdf1arr[id] = beamAPtr->xfHard(id, x1Save, Q2FacSave);
    pdf2arr[id] = beamBPtr->xfHard(id, x2Save, Q2FacSave);
  }
  ...
  if (nFinal == 1)
  {
    if (convertM2) 
    {
      sigmaSum /= 2. * sH; // TODO: what is sH?
      // Convert 2 * pi * delta(p^2 - m^2) to Breit-Wigner with same area.
      // TODO: why do we assume resonance just based on units?
      double mTmp   = particleDataPtr->m0(resonanceA);
      double GamTmp = particleDataPtr->mWidth(resonanceA);
      sigmaSum *= 2. * mTmp * GamTmp / ( pow2(sH - mTmp * mTmp) + pow2(mTmp * GamTmp) );
    }
  }

  if (nFinal == 2)
  {
    if (convertM2) sigmaSum /= 16. * M_PI * sH2;
  }

  // SigmaProcess
  if (convert2mb) sigmaSum *= CONVERT2MB;
...
void Sigma2qqbar2squarkantisquark::initProc() {
  ...
  // Derive name
  name = "q qbar' -> "+particleDataPtr->name(abs(id3Sav))+" "+
...
void Sigma2gg2squarkantisquark::initProc() {
  ...
  // Process Name
  name = "g g -> "+particleDataPtr->name(abs(id3Sav))+" "
    +particleDataPtr->name(-abs(id4Sav));
...
void Sigma2qg2squarkgluino::initProc() {
  ...
  // Derive name
  name = "q g -> "+particleDataPtr->name(abs(id3Sav))+" gluino + c.c.";
...
void Sigma1qq2antisquark::initProc(){
  ...
  name = "q q' -> " + particleDataPtr->name(-idRes)+" + c.c";
